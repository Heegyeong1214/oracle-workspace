...
--4000:크기 초과(3자리까지만 가능-0~999) /음수도 가능
INSERT INTO student(name,age,avg,hire)
VALUES('홍길동', 4000, 97.85,sysdate);

--ORA-01438: value larger than specified precision allowed for this column
--2921.8589: 크기 초과(5자리까지만 가능, 정수는 3, 소수점은 2)-소수점은 포함하지 않음
INSERT INTO student(name, age, avg,hire)
VALUES('홍길동', 30, 2921.8589,sysdate);

--정수는 설정한 값과 일치하고 소수점자리수가 초과하면 초과한 개수는 잘려서 저장한다.
INSERT INTO student(name, age, avg,hire)
...
--ORA-01438: value larger than specified precision allowed for this column
--소수점 2자리로 무조건 계산되고, 정수 자리는 3자리까지만 허용한다.
INSERT INTO student(name, age, avg,hire)
...
--테이블에 컬럼을 추가한다.
--제약조건을 주지 않았기 때문에 NOT NULL로 하면 안된다.
ALTER TABLE student
ADD loc varchar2(50);
...
--5보다 큰 데이터가 이미 삽입되어 있는 상황이라 크기를 줄여도 에러가 뜸
--크기를 줄이는 것은 지양, 크기를 키우는 것은 상관 없음
ALTER TABLE student
...
DROP : 테이블 자체를 제거한다.
DELETE : 테이블에 저장된 데이터를 모두 삭제한다.(auto COMMIT이 안됨) -> Rollback 가능해서 되돌릴 수 있음
==================================================*/

--커밋하면 기존의 트랜잭션이 종료되서 새로운 트랜잭션 시작
commit;
...
--ORA-02290: check constraint (HR.EMP04_AGE_CK) violated
 ...
 --NOT NULL은 컬럼LEVEL에서만 가능해서 변경(MODIFY)을 해야 함
 ALTER TABLE emp04
 ADD CONSTRAINT emp04_name_nn not null(sal);
 
 -- NOT NULL 추가는 MODIFY로 설정한다.
 ALTER TABLE emp04
 MODIFY name NOT NULL;
 
 --ORA-01400: cannot insert NULL into ("HR"."EMP04"."NAME")
--NULL이 아닌데 NULL이라고 입력했으니 오류가 뜨는게 맞음
 INSERT INTO emp04(id, name, age, gen)
 VALUES(5, null, 35, 'm'); 
...
--empno,deptno 컬럼제약조건 locno 테이블 제약조건
 CREATE TABLE emp07(
...
--현재 다른 테이블에서 deptno를 참조하고 있어서 삭제 안됨
  DELETE FROM dept01
 WHERE deptno=20;
...
--dept01이 업데이트가 된 후 deptno가 20이면 50으로 설정해라
 BEGIN
  UPDATE emp07
  SET deptno=50
  WHERE deptno=20;
