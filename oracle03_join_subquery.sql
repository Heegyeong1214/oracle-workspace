...
--1) oracle용 cartesian product
  SELECT e.department_id, e.first_name, e.job_id, j.job_title --employee에 있는 job_id
...
--1400,1500번 위치의 도시 이름과 그 곳에 있는 부서의 이름을 출력하시오
SELECT l.city, d.location_id, d.department_name
FROM locations l, departments d
WHERE l.location_id = d.location_id AND d.location_id IN(1400,1500);

SELECT l.city, d.location_id, d.department_name
FROM locations l INNER JOIN departments d
ON l.location_id = d.location_id AND d.location_id IN(1400,1500);
...
FROM employees e, jobs j               
WHERE  e.job_id=j.job_id
AND e.salary>=j.min_salary AND e.salary<=j.max_salary;
...
SELECT department_name
FROM departments
WHERE department_id = (                 -- =를 표현하면 서브쿼리의 값은 하나여야 한다.
                        SELECT department_id
                        FROM employees
                        WHERE first_name = 'Lex'
                        );
...
SELECT department_id   --중복이 허용되는 컬럼인지 확인해야함 => 이 결과가 2개(50,90)이라 등호를 사용해서 서브쿼리를 작성할 수 없어 IN활용
                    FROM employees
...
SELECT department_id,max( salary) -- 직계함수랑 일바함수랑 같이 사용 못함
FROM employees
GROUP BY department_id;

SELECT first_name, department_id, salary
FROM employees
WHERE (department_id, salary) IN (SELECT department_id,max( salary) -- 직계함수랑 일바함수랑 같이 사용 못함
                                FROM employees
                                GROUP BY department_id
                                );
...
SELECT department_id, department_name   --6
FROM departments d                       --1
WHERE department_id IN (                    --5
                    SELECT department               --4
                    FROM employees              --2
                    WHERE department_id = d.department_id   --3
                    );
...
SELECT department_id, department_name
FROM departments d
WHERE EXISTS (      --EXIST는 TRUE가 있는 것을 확인하는 용도, 만약 false면 메인쿼리에서 안가지고 옴
                    SELECT 1
                    FROM employees
                    WHERE department_id = d.department_id   --true이면 메인쿼리의 SELECT 호출가능
...
SELECT department_id, department_name
FROM departments d
WHERE NOT EXISTS (      --EXIST는 TRUE가 있는 것을 확인하는 용도, 만약 false면 메인쿼리에서 안가지고 옴
                    SELECT 1
                    FROM employees
                    WHERE department_id = d.department_id   --true이면 메인쿼리의 SELECT 호출가능
...
WHERE NOT EXISTS(
            ...
            /*=========================================================
Top-N 서브쿼리: 페이징 처리할 때 사용한다
   상위의 값을 추출할때 사용된다.
   <, <=연산자를 사용할수 있다. 단 비교되는 값이 1일때는 =도 가능하다.
   order by절을 사용할 수 있다.
=========================================================*/
--급여가 가장 높은 상위 3명을 검색하시오.
SELECT rownum, emp.first_name,emp.salary /*rownum은 select의 출력 순서*/
FROM(
...
--롤업 2개: Department ID당 총계, 전체 총계
 SELECT department_id, job_id, count(*)
 FROM employees
 GROUP BY rollup(department_id, job_id)
 ORDER BY department_id, job_id;

--롤업 2개인데 괄호 두개 있으면 전체 총계
 SELECT department_id, job_id, count(*)
 FROM employees
 GROUP BY rollup((department_id, job_id))
 ORDER BY department_id, job_id;
...
--일반 데이터라서 0을 반환하고 합계/소계에 대한 107은 1을 반환함
