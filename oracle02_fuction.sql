...
--ORA-12899: value too large for column "HR"."USER1"."DATA" (actual: 11, maximum: 5)
 ...
 -- data varchar2가 5인데 바이트가 5를 초과하니까 오류가 뜨는 것
 INSERT INTO user1(data)
 VALUES('한국');
 ...
 --뒤에서 부터 3번째에서 4개 추출: e부터 3개밖에 없으니까 est 추출
...
-- lpad('문자', 총 길이,'왼쪽여백을 채워줄 문자'); 
SELECT lpad('korea',8,'*')
FROM dual;

-- rpad('문자', 총 길이,'오른쪽여백을 채워줄 문자');
SELECT rpad('korea',8,'*')
FROM dual;
...
--남은 자리수는 0으로 채워줌
SELECT to_char(2532, '000,000.00') /* 002,532.00*/
FROM dual;
...
FROM employees
ORDER BY department_id ASC; --ASC는 생략 가능함

--직급이 'PR_REP' 인 사원은 급여의 5%, 'SA_MAN'인 사원은 10%, 
...
==================================================*/
--max(컬럼): 최대값
--단순 컬럼은 값이 여러개인데 그룹함수는 값이 하나라 같이 사용할 수 없음
SELECT max(salary)
FROM employees;
...
--컬럼 명을 ()에 넣어주면 null이 아닌 갯수를 추출한다
